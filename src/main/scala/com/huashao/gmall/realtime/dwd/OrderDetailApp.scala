...
import com.huashao.gmall.realtime.bean.{OrderDetail, SkuInfo}
import com.huashao.gmall.realtime.util.{MyKafkaSink, MyKafkaUtil,
  OffsetManagerUtil, PhoenixUtil}
        ...
        rdd.foreach{
          orderDetail=>{
            MyKafkaSink.send("dwd_order_detail",
              /*
              把对象转为json字符串，因为kafka的send方法要传入的是String；而里面的SerializeConfig会递归
              地序列化对象和里面的所有属性。
               */
